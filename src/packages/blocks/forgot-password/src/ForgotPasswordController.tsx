...
> {
  ...
  async receive(from: string, message: Message) {
    ...
    if (apiRequestCallId && responseJson) {
      if (apiRequestCallId === this.apiForgotCallId) {
        if(responseJson?.errors) {
          this.setState({otpError: true})
          this.setState({errorValue: "Account Not Found"})
        } else {
          ...
          responseJson && this.props.history.push("/ForgotPasswordOTPs");
          // responseJson && this.props.navigation.navigate("ForgotPasswordOTP1")
        }
      }
      if (apiRequestCallId === this.apiVerifyotpCallId) {
          if(responseJson?.errors){
            ...
            if(this.state.otpVal === "" ) {
              this.setState({errorValue: "Please enter OTP code"})
              console.clear()
              console.log("PLEASE ENTER OTP CODE ")
            }
            else {
              console.clear()
              this.setState({errorValue: "Please enter valid OTP code"})
              console.log("OTP ERROR")
            }
            // console.log(responseJson.errors)
          }
          else {
            responseJson && this.props.history.push("/NewPasswords");
            // responseJson && this.props.navigation.navigate("NewPassword1")
          }
      }
    ...
    if (apiRequestCallId === this.apiSetNewPasswordCallId) {
      if (responseJson ) {
        runEngine.unSubscribeFromMessages(this, this.subScribedMessages);
       if(responseJson.errors){
          alert(responseJson.errors[0].password)
        }
        if (responseJson?.meta?.message){

          this.setState({newCompo:true})
        }else{
...
verifyOtp = () => {
  ...
  // let otp = this.state.otpValue1+this.state.otpValue2+this.state.otpValue3+this.state.otpValue4+this.state.otpValue5+this.state.otpValue6;
  // console.log(otp)
  let otp = this.state.otpVal
  ...
  requestMessage.addData(
    getName(MessageEnum.RestAPIResponceEndPointMessage),
    configJSON.otpConfirmationAPIEndPoint
  );
