...
export default class  DesignationController extends BlockComponent<
  Props,
  S,
  SS
> {
    ...
    if (getName(MessageEnum.RestAPIResponceMessage) === message.id) {
      ...
      if (apiRequestCallId === this.updateDesignationApiCallId) {
        ...
        if (responseJson && !responseJson.errors && responseJson) {
          if (responseJson.length === 0 || responseJson.message) {
            this.setState({
              errorMsg: "Data Not Found",
              loading: false,
              modalMsg:"Designation Already Exist.",
              isSuccessModal:true
            });
          } else {
            ...
            // const rowData = this.state.designationData.filter((item:any) => item.id!= apiData.id)
            // this.setState(
            //   {
            //     // designationData: responseJson.data.attributes,
